<template>
  <q-page padding>
    <div class="oslo-page-header">
      ...
      <div class="col-auto">
        <q-btn v-permissao="'GRUPO_CRIAR'" color="primary" unelevated no-caps icon="add" label="Novo Grupo"
          @click="abrirModal()" />
      </div>
    </div>
      ...
      <q-card-section class="q-py-sm">
        <div class="row q-col-gutter-md items-center">
          <div class="col-md-6 col-12">
            <q-input v-model="busca" outlined dense placeholder="Buscar por nome ou descrição..." clearable
    ...
    <q-card flat bordered>
      <q-table flat :rows="grupos" :columns="colunas" :loading="loading" row-key="id"
        ...
        <template #body-cell-ativo="props">
          <q-td :props="props" class="text-center">
            <q-badge :color="props.row.ativo ? 'positive' : 'grey-5'" :label="props.row.ativo ? 'Ativo' : 'Inativo'" />
          </q-td>
        </template>

        <template #body-cell-acoes="props">
          <q-td :props="props" class="text-center">
            <q-btn v-permissao="'GRUPO_EDITAR'" flat round dense icon="eva-edit-2-outline" color="grey-7" size="sm"
              @click="abrirModal(props.row)">
              <q-tooltip>Editar</q-tooltip>
            </q-btn>
            <q-btn v-permissao="'GRUPO_EXCLUIR'" flat round dense icon="eva-trash-2-outline" size="sm" color="negative"
    ...
    <!-- Modal -->
    <grupo-show v-model="showModal" @salvo="carregarGrupos" />
  </q-page>
</template>
...
const colunas = [
  ...
  { name: "ativo", label: "Status", field: "ativo", align: "center", style: "width: 100px" },
...
const abrirModal = async (row = null) => {
  if (row?.id) {
    await grupoStore.show(row.id);
  }
