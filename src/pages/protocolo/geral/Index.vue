  ...
  <div class="row q-col-gutter-md q-pa-md">
      ...
      <q-card bordered class="q-mb-md rounded-borders">
        <q-card-section class="q-pa-none">
          <div class="ds-header">
            ...
            <q-space />
            <q-chip :color="getStatusColor(protocolo?.status)" :label="protocolo?.status || 'Pendente'"
              text-color="white" size="sm" class="rounded-borders" outline />
          </div>
        </q-card-section>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td class="ds-table-value">
                  ...
                  <div v-if="protocolo?.cliente?.cpf_cnpj" class="text-caption text-grey-6">
      ...
      <q-card bordered class="rounded-borders ds-anotacoes-melhoradas">
        <q-card-section class="q-pa-none">
          <div class="ds-header">
            ...
            <q-space />
            <q-chip v-if="protocolo?.anotacoes?.length > 0" :label="protocolo.anotacoes.length" color="blue-grey-3"
              text-color="blue-grey-8" size="sm" class="rounded-borders q-mr-sm" />
            <q-btn icon="eva-plus" flat round size="sm" color="primary" @click="toggleNovaAnotacao"
        ...
        <q-card-section class="q-pa-md">
          ...
          <q-slide-transition>
            <div v-show="mostrandoNovaAnotacao" class="q-mb-md ds-nova-anotacao">
              <editor-basico v-model="novaAnotacao" :altura="120" id="ide-minuta"
                placeholder="Digite sua anotação..." />
              <div class="row justify-end q-mt-sm q-gutter-xs">
                <q-btn label="Cancelar" flat size="sm" @click="cancelarAnotacao" class="rounded-borders" />
                <q-btn label="Publicar" color="primary" size="sm" icon="eva-paper-plane-outline"
                  @click="publicarAnotacao" :disable="!novaAnotacao?.trim()" class="rounded-borders" />
              </div>
            </div>
          </q-slide-transition>
          ...
          <div v-if="protocolo?.anotacoes?.length > 0" class="ds-annotations">
            ...
            <div class="ds-annotations-container">
              <div v-for="anotacao in protocolo.anotacoes" :key="anotacao.id" class="ds-annotation-item">
                <div class="row items-center q-gutter-sm q-mb-xs">
                  ...
                  <div class="row q-gutter-xs anotacao-actions">
                    <q-btn icon="eva-edit-outline" flat round size="sm" color="grey-7"
                      @click="editarAnotacao(anotacao)">
                      <q-tooltip>Editar</q-tooltip>
                    </q-btn>
                    <q-btn icon="eva-trash-2-outline" flat round size="sm" color="grey-7"
                      @click="confirmarExclusao(anotacao)">
                      <q-tooltip>Excluir</q-tooltip>
                    </q-btn>
                  </div>
                </div>
                <div class="text-body2 text-grey-8 anotacao-conteudo" v-html="anotacao.anotacao" />
              </div>
            </div>
          </div>
          ...
          <div v-else class="text-center text-grey-6 q-py-md">
            <q-icon name="eva-message-square-outline" size="3em" class="q-mb-sm" />
    ...
    <div class="col-md-4 col-sm-12 col-xs-12">
      ...
      <q-card bordered class="q-mb-md rounded-borders">
        <q-card-section class="q-pa-none">
          <div class="ds-header">
            ...
            <q-space />
            <q-btn label="Encaminhar" outline color="primary" size="sm" icon="eva-arrow-forward-outline"
              @click="mostrarModalNovoOficio" class="rounded-borders" />
          </div>
        </q-card-section>
        ...
        <q-card-section class="q-pa-md">
          <div class="ds-process-item">
            <div class="row items-center q-gutter-sm">
              ...
              <q-btn icon="eva-arrowhead-right-outline" outline color="green-6" />
            </div>
          </div>
        </q-card-section>
      </q-card>
      ...
      <q-card bordered class="rounded-borders">
        ...
        <q-card-section class="q-pa-md">
          <div v-if="protocolo?.etapa?.etapas_seguinte?.length > 0" class="column q-gutter-sm">
            <q-btn v-for="etapa in protocolo.etapa.etapas_seguinte" :key="etapa.id" outline color="green-6" no-caps
              class="text-left justify-start rounded-borders" size="sm" @click="
          ...
          <div v-else class="text-center text-grey-6 q-py-md">
            <q-icon name="eva-checkmark-circle-outline" size="2em" color="green-6" class="q-mb-sm" />
