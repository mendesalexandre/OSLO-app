<template>
  <modal
    v-model="model"
    titulo="Calculadora de Emolumento"
    :subtitulo="ato?.nome ?? ''"
    tamanho="sm"
    @close="fechar"
  >
    ...
    <div class="row q-col-gutter-md">
      <div class="col-12">
        ...
        <q-input
          ...
          :disable="ato?.tipo_calculo === 'FIXO'"
          :hint="ato?.tipo_calculo === 'FIXO' ? 'Ato de valor fixo — base não é necessária' : ''"
        />
      </div>
...
const calcular = async () => {
  ...
  resultado.value  = null
  try {
    resultado.value = await tabelaCustaAtoStore.calcular(props.ato.uuid, calcForm.value)
  } catch (e) {
